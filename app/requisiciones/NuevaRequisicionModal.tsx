      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto font-sans p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs">
            <div>
              <label className="font-bold text-slate-700 block mb-1">Solicitante *</label>
              <input
                type="text"
                required
                value={solicitante}
            ...
            <div>
              <label className="font-bold text-slate-700 block mb-1">Departamento / Área</label>
              <select
                value={departamento}
            ...
            <div>
              <label className="font-bold text-slate-700 block mb-1">Prioridad Operativa</label>
              <select
                value={prioridad}
          ...
          <div>
            <label className="font-bold text-slate-700 text-xs block mb-1">Motivo o Justificación de la Compra</label>
            <textarea
              rows={2}
              value={motivo}
              onChange={(e) => setMotivo(e.target.value)}
              placeholder="Ej. Desgaste en producción de flechas OT-9940. Requerido para evitar paro de máquina."
              className="w-full px-3 py-2 text-xs border border-slate-300 rounded-lg bg-white placeholder:text-slate-400"
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {items.map((it, idx) => (
                <div key={idx} className="p-3.5 bg-white border border-slate-300 rounded-xl shadow-2xs space-y-3">
                  <div className="flex items-center justify-between text-xs border-b border-slate-100 pb-2">
                    <span className="font-bold font-mono text-slate-600">Herramienta #{idx + 1}</span>
                    {items.length > 1 && (
                      <button
                        type="button"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
                    <div className="sm:col-span-2">
                      <label className="font-bold text-slate-700 block mb-1">Descripción del Ítem *</label>
                      <input
                        type="text"
                        required
                    ...
                    <div className="flex gap-2">
                      <div className="w-1/2">
                        <label className="font-bold text-slate-700 block mb-1">Cant.</label>
                        <input
                          type="number"
                          min={1}
                          value={it.cantidad}
                          onChange={(e) => updateItem(idx, 'cantidad', Number(e.target.value))}
                          className="w-full px-2 py-1.5 border border-slate-300 rounded-lg text-xs font-mono font-bold"
                        />
                      </div>
                      <div className="w-1/2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                    <div>
                      <label className="font-semibold text-slate-600 block mb-1">Marca Preferida</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="font-semibold text-slate-600 block mb-1">Proveedor Sugerido (Catálogo)</label>
                      <select
                    ...
                    <div>
                      <label className="font-semibold text-slate-600 block mb-1">Especificación Taller</label>
                      <input
                        type="text"
            ...
            {aprobacionRequerida && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 border border-purple-300 rounded-lg bg-white text-xs font-medium"
                />
              </div>
            )}
          </div>
