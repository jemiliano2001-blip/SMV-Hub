...
export default function SyncSheetSection({ onSyncCompletada }: SyncSheetSectionProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-5 shadow-xs transition-all duration-200 hover:shadow-md">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0 self-end lg:self-center">
            <button
              ...
              className="inline-flex items-center gap-1 text-xs font-semibold text-muted-foreground hover:text-foreground bg-muted hover:bg-muted px-3 py-2 rounded-lg transition-colors"
            >
        ...
        {mostrarGuia && (
          <div className="mt-4 p-4 rounded-lg bg-sky-50 border border-sky-200 text-xs text-sky-900 space-y-2 animate-in fade-in slide-in-from-top-2">
            ...
            <ol className="list-decimal list-inside space-y-1 pl-1 text-foreground">
              ...
              <li>Haz clic en <strong>Publicar</strong> y copia el enlace generado (ejemplo: <code className="bg-card px-1 py-0.5 rounded font-mono text-[11px] border border-sky-200">https://docs.google.com/spreadsheets/d/e/.../pub?output=csv</code>).</li>
        ...
        {/* Config URL Form */}
        <form onSubmit={handleGuardarUrl} className="mt-4 flex flex-col sm:flex-row gap-2 pt-3 border-t border-border">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-input bg-muted pl-9 pr-3 py-1.5 text-xs font-mono text-foreground placeholder:text-muted-foreground focus:bg-card focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 transition-colors"
            />
          </div>
          <button
            type="submit"
            disabled={guardandoUrl || urlInput.trim() === config?.urlSheet}
            className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-input bg-card px-4 py-1.5 text-xs font-bold text-foreground hover:bg-muted disabled:opacity-40 transition-colors shrink-0"
          >
